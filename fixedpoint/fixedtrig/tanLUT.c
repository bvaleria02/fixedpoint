...
#define TAN_F64_FRACT_MASK 0x3FFFFF
...
fixed32_t abstractTanLUT32(fixed32_t angle){
	...
	*/
	...
	if(index1 >= (TAN_F32_COUNT - 1) && index0 >= (TAN_F32_COUNT - 1)){
		fixedSetErrno(FP_ERROR_INFINITY);
		y1 = I32_MANTISSA;
	} else {
		y1 = tanLUTF32[index1];
	};
...
fixed64_t abstractTanLUT64(fixed64_t angle){
	...
	*/
	...
	uint16_t index0 = angle >> (F64_RSHIFT - TAN_F64_SHIFT);
	uint16_t index1 = index0 + 1;

	// If (for some reason) the index is greater than the element count, return the last one
	if(index0 >= TAN_F64_COUNT){
		index0 = TAN_F64_COUNT - 1;
	}

	if(index1 >= TAN_F64_COUNT){
		index1 = TAN_F64_COUNT - 1;
	}
	...
	*/
	...
	ufixed64_t x = angle & TAN_F64_FRACT_MASK;
	x = x << TAN_F64_SHIFT;

	/*
		Linear interpolation
		fixedLerp64(y0: initial point, y1: final point, x: fractional part)
	*/
	fixed64_t y0 = tanLUTF64[index0];

	// Uses the max value for fixed64_t if both index are equal to the LUT count
	fixed64_t y1;
	if(index1 >= (TAN_F64_COUNT - 1) && index0 >= (TAN_F64_COUNT - 1)){
		fixedSetErrno(FP_ERROR_INFINITY);
		y1 = I64_MANTISSA;
	} else {
		y1 = tanLUTF64[index1];
